const POINTS_PER_SQUARE = 20;
const SHOW_RADIOUS = 2;
const POINTS_COLOR = "#ffffff3d";
const POINTS_COLOR_VARIABLE = "rgba(255, 255, 255,";
...
function initHeader() {
    ...
    for (let i in points) {
        let c = new Circle(points[i], 2 + Math.random() * 2, POINTS_COLOR);
        points[i].circle = c;
    }
}
    ...
    if (animateHeader) {
        ...
        for (let i in points) {
            ...
            if (Math.abs(getDistance(target, points[i])) < SHOW_RADIOUS * 10000) {
                ...
                SHOW_RADIOUS * 10000 * 2
            ) {
                points[i].active = 0.1;
                points[i].circle.active = 0.3;
            } else if (
                Math.abs(getDistance(target, points[i])) <
                SHOW_RADIOUS * 10000 * 3
            ) {
